...
print("Testing history log file without numbers after date...")
print("=" * 80)
...
if os.path.exists(error_log_path):
    ...
    print(f"\nCurrent error.log size: {current_size} bytes")
else:
    print("\nerror.log does not exist yet")
    ...
    if history_files:
        ...
        if error_history_files:
            ...
            for file in error_history_files:
                print(f"  - {file}")
                # 检查文件名是否包含日期信息且数字后缀符合要求
                parts = file.split(".")
                if len(parts) >= 3:
                    # 检查最后一部分是否为8位数字日期
                    if len(parts[-1]) == 8 and parts[-1].isdigit():
                        # 第一个文件，应该没有数字后缀
                        print(f"    ✅ No number suffix after date")
                    # 检查倒数第二部分是否为8位数字日期
                    elif len(parts[-2]) == 8 and parts[-2].isdigit():
                        # 检查是否有数字后缀
                        if len(parts) > 3:
                            last_part = parts[-1]
                            if last_part.isdigit():
                                # 第二个及以后的文件，应该有数字后缀
                                print(f"    ✅ Contains number suffix after date: {last_part}")
                            else:
                                print(f"    ✅ No number suffix after date")
                        else:
                            # 第一个文件，应该没有数字后缀
                        ...
                        print(f"    ❌ Does NOT contain valid date information")
                        success = False
                else:
...
if success:
    print("✅ Test passed! History log files have correct number suffixes after date.")
else:
    print("❌ Test failed! Some history log files have incorrect number suffixes after date.")

print("History log file without numbers after date test completed!")
