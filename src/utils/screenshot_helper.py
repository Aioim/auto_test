...
class ScreenshotHelper:
    ...
    def compare_screenshots(
        ...
        Args:
            ...
            current_path: 当前截图路径
            output_path: 差异结果保存路径（可选）
            threshold: 差异阈值（0-1），默认 0.01
        Returns:
            Dict[str, Any]: 比较结果，包含以下字段：
                - are_same: 是否相同
        ...
        if not current_path.exists():
            raise ValueError(f"Current screenshot not found: {current_path}")
        # 打开截图
        with Image.open(baseline_path) as baseline_img, Image.open(current_path) as current_img:
            ...
            if baseline_img.size != current_img.size:
                return {
                    ...
                    "output_path": None,
                    "message": "Screenshots have different sizes"
                }
            # 转换为灰度图
            baseline_gray = baseline_img.convert("L")
            current_gray = current_img.convert("L")
            # 计算差异
            baseline_array = np.array(baseline_gray)
            current_array = np.array(current_gray)
            diff_array = np.abs(baseline_array - current_array)
            # 计算差异比例
            total_pixels = diff_array.size
            different_pixels = np.sum(diff_array > 10)  # 阈值为 10，忽略微小差异
            difference_ratio = different_pixels / total_pixels
            are_same = difference_ratio < threshold
            # 生成差异图像
            if output_path or not are_same:
                if not output_path:
                    output_path = current_path.parent / f"{current_path.stem}_diff{current_path.suffix}"
                else:
...
if __name__ == '__main__':
    ...
    with sync_playwright() as p:
        ...
        # 截取可视区域
        meta1 = helper.take_viewport_screenshot(name="viewport_demo")
        print(f"✓ 可视区域截图：{meta1.filepath}")
        
        # 截取完整页面
        meta2 = helper.take_full_page_screenshot(name="fullpage_demo")
        print(f"✓ 完整页面截图：{meta2.filepath}")
        
        # 指定格式和质量
        meta3 = helper.take_screenshot(
            name="quality_demo",
            format=ScreenshotFormat.JPEG,
            quality=80
        )
        print(f"✓ JPEG 格式截图：{meta3.filepath}")
        ...
        # 截取特定元素
        try:
            meta4 = helper.take_element_screenshot(
                selector="#s-hotsearch-wrapper",
                name="element_h1"
            )
            print(f"✓ 元素截图 (h1)：{meta4.filepath}")
        except Exception as e:
            print(f"✗ 元素截图失败：{e}")
        
        # 高亮元素并截图
        try:
            meta5 = helper.highlight_and_capture(
                selector="#s-hotsearch-wrapper",
                name="highlighted_h1",
                color="blue",
                thickness=5
            )
            print(f"✓ 高亮元素截图：{meta5.filepath}")
        except Exception as e:
            print(f"✗ 高亮截图失败：{e}")
        ...
        # 使用上下文管理器高亮
        try:
            with helper.highlighted_context("#s-hotsearch-wrapper", color="green", thickness=3):
                page.wait_for_timeout(500)  # 保持高亮状态
                helper.take_screenshot(name="context_highlight")
            print("✓ 上下文管理器高亮完成")
        except Exception as e:
            print(f"✗ 高亮上下文失败：{e}")
        ...
        # 手动高亮和移除
        try:
            helper.highlight_element("#s-hotsearch-wrapper", color="red")
            page.wait_for_timeout(300)
            count = helper.remove_highlight()
            print(f"✓ 手动高亮完成，移除 {count} 个高亮元素")
        except Exception as e:
            print(f"✗ 手动高亮失败：{e}")
        
        # ==================== 4. 标注功能 ====================
        print("\n【4】标注功能 (annotate_screenshot)")
        
        # 示例 4.1：添加文本标注
        try:
            meta_anno1 = helper.annotate_screenshot(
                name="annotated_text",
                annotations=[
                    {
                        "type": "text",
            ...
            print(f"✓ 文本标注截图：{meta_anno1.filepath}")
        except ImportError as e:
            print(f"✗ 标注功能需要 Pillow 库：{e}")
        except Exception as e:
            print(f"✗ 文本标注失败：{e}")
        
        # 示例 4.2：添加矩形框标注
        try:
            meta_anno2 = helper.annotate_screenshot(
                name="annotated_rectangle",
                annotations=[
                    {
                        "type": "rectangle",
            ...
            print(f"✓ 矩形标注截图：{meta_anno2.filepath}")
        except Exception as e:
            print(f"✗ 矩形标注失败：{e}")
        
        # 示例 4.3：添加箭头标注
        try:
            meta_anno3 = helper.annotate_screenshot(
                name="annotated_arrow",
                annotations=[
                    {
                        "type": "arrow",
            ...
            print(f"✓ 箭头标注截图：{meta_anno3.filepath}")
        except Exception as e:
            print(f"✗ 箭头标注失败：{e}")
        ...
        try:
            annotations = [
                ...
                {
                    "type": "arrow",
                    "position": (350, 100, 450, 100),
                    "color": "blue"
                }
            ]
            meta_anno4 = helper.annotate_screenshot(
                name="annotated_complex",
                annotations=annotations,
                screenshot_type=ScreenshotType.VIEWPORT
            )
            print(f"✓ 混合标注截图：{meta_anno4.filepath}")
            print(f"  标注数量：{len(meta_anno4.annotations)}")
        except Exception as e:
            print(f"✗ 混合标注失败：{e}")
        ...
        try:
            # 截取两张截图进行比较
            base_path = helper.take_screenshot(name="compare_base").filepath
            page.reload()
            current_path = helper.take_screenshot(name="compare_current").filepath
            
            result = helper.compare_screenshots(
                baseline_path=base_path,
                current_path=current_path,
                threshold=0.05
            )
            print(f"✓ 截图比较结果：")
            print(f"  是否相同：{result['are_same']}")
            print(f"  差异比例：{result['difference_ratio']:.2%}")
            if result['output_path']:
                print(f"  差异图：{result['output_path']}")
        except Exception as e:
            print(f"✗ 截图比较失败：{e}")
        ...
        print(f"✓ 截图历史数量：{len(history)}")
        
        # 获取最新截图
        latest = helper.get_latest_screenshot()
        if latest:
            print(f"✓ 最新截图：{latest.name} ({latest.filepath})")
        
        # 导出历史记录
        try:
            helper.export_history("output/screenshots/history.json")
            print("✓ 历史记录已导出到 JSON")
        except Exception as e:
            print(f"✗ 导出历史失败：{e}")
        ...
        # 获取 Base64 编码
        try:
            base64_img = helper.get_screenshot_as_base64()
            print(f"✓ Base64 编码长度：{len(base64_img)} 字符")
        except Exception as e:
        ...
        # 快速截图
        try:
            path1 = take_screenshot(page, name="quick_shot")
            print(f"✓ 便捷截图：{path1}")
        except Exception as e:
            print(f"✗ 便捷截图失败：{e}")
        ...
        # 高亮并截图
        try:
            path2 = highlight_and_screenshot(page, selector="#s-hotsearch-wrapper", name="quick_highlight")
            print(f"✓ 便捷高亮截图：{path2}")
        except Exception as e:
            print(f"✗ 便捷高亮截图失败：{e}")
        ...
        # 清理旧截图（保留最新 10 个）
        deleted = helper.cleanup_screenshots(keep_latest=10)
        print(f"✓ 清理完成，删除 {deleted} 个旧截图")
