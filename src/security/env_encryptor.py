...
def decrypt_env_key(env_var: str) -> str:
    """已弃用：使用 fetch_and_decrypt_env_var 替代"""
    import warnings
    warnings.warn(
...
def encrypt_env_file(input_path: str, output_path: Optional[str] = None):
    """已弃用：使用 process_env_file 替代"""
    import warnings
    warnings.warn(
        "'encrypt_env_file' is deprecated, use 'process_env_file' instead.",
        DeprecationWarning,
        stacklevel=2,
    )
    return process_env_file(input_path, output_path)
