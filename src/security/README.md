...
├── secure_env_loader.py # 安全 .env 文件加载器
├── env_encryptor.py     # 环境变量加密工具
├── key_rotator.py       # 密钥轮换与管理
├── demo.py              # 模块使用示例
└── README.md            # 模块文档（本文件）
```
...
```python
from security import SecureEnvLoader, load_secure_dotenv
...
```python
from security import encrypt_value, decrypt_value, encrypt_env_file
...
```python
from security import KeyRotator, perform_key_rotation
...
```python
from security import SecretStr, get_secret
...
| `load_secure_dotenv()` | 安全加载 .env 文件 | `dotenv_path=None`, `override=False` |
| `encrypt_value()` | 加密单个值 | `value` |
| `decrypt_value()` | 解密单个值 | `value` |
| `encrypt_env_file()` | 加密 .env 文件 | `input_file`, `output_file=None` |
| `perform_key_rotation()` | 执行密钥轮换 | `backup_dir=None`, `env_files=None`, `dry_run=False` |
| `generate_key_file()` | 生成密钥文件 | `key_path=None`, `force=False` |
| `mask_value()` | 掩码敏感值 | `value`, `show_chars=2` |
| `format_safely()` | 安全格式化字符串 | `template`, `**kwargs` |
...
```python
import unittest
from security import SecretStr, secrets
...
class TestSecurityModule(unittest.TestCase):
    ...
    def test_secrets_manager(self):
        ...
        secret = secrets.get_secret("test_key")
...
```python
from security import secrets, KeyRotator

def test_security_audit():
    """测试安全审计功能"""
    # 测试密钥访问记录
    secrets.set_secret("audit_test", "audit_value")
    secret = secrets.get_secret("audit_test")
    print("密钥访问测试完成")
...
### 更新日志
- **v2.0.0** (2026-04-10): 重构版本，改进模块结构和API设计
